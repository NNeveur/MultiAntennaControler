...
#define PWM1  3 //Motor Enamble pin Runs on PWM signal
#define InA1  4  // Motor Forward pin
#define InB1  5 // Motor Reverse pin
#define PWM2  10 // Motor2 Enamble pin Runs on PWM signal
#define InA2  8  // Motor2 Forward pin
#define InB2  9 // Motor2 Reverse pin
#define LOOPTIME        1                    // PID loop time
...
#define LOW2         -64
#define HIGH2         64
int encoder1Pin1 = 2; //motor1 Encoder Output 'A' must connected with intreput pin of arduino every int0.
int encoder1Pin2 = 6; //motor1 Encoder Output 'B' must connected with intreput pin of arduino every int44.
int encoder2Pin1 = 7; //motor2 Encoder Output 'A' must connected with intreput pin of arduino every int1.
int encoder2Pin2 = 11; //motor2 Encoder Output 'B' must connected with intreput pin of arduino every int32.
...
double consKp = 1, consKi = 0.01, consKd = 0.2;    // Conservative for near final position
...
void setup() {
  ...
  Serial.begin(9600);
  Serial1.begin(9600);
  myPID1.begin(&input1, &output1, &setpoint1, consKp, consKi, consKd);  
  myPID2.begin(&input2, &output2, &setpoint2, consKp, consKi, consKd);  
...
//  myPID1.SetMode(AUTOMATIC);   //set PID in Auto mode
  ...
  myPID1.setOutputLimits(LOW1, HIGH1); // this is the MAX PWM value to move motor, here change in value reflect change in speed of motor.
//  myPID1.start();
//  myPID2.SetMode(AUTOMATIC);   //set PID in Auto mode
  myPID2.setSampleTime(LOOPTIME);  // refresh rate of PID controller
  myPID2.setOutputLimits(LOW1, HIGH1); // this is the MAX PWM value to move motor, here change in value reflect change in speed of motor.
//  myPID1.start();
...
void updateEncoder1(){
  ...
  if (run1) {
    ...
    {  //we're close to setpoint, use conservative tuning parameters
      myPID1.setOutputLimits(LOW2, HIGH2); // this is the MAX PWM value to move motor, here change in value reflect change in speed of motor.
      myPID1.setCoefficients(consKp, consKi, consKd);
    } else {
     //we're far from setpoint, use aggressive tuning parameters
      myPID1.setOutputLimits(LOW1, HIGH1); // this is the MAX PWM value to move motor, here change in value reflect change in speed of motor.
      myPID1.setCoefficients(aggKp, aggKi, aggKd);
    }
    myPID1.compute();
    PWM_val1 = output1;
    if (PWM_val1 >=0) motorForward1(PWM_val1);
...
void updateEncoder2(){
  ...
  if (run2) {
    ...
    if ((abs(lastPosition2)) == tickNumber2) {
      ...
      if (gap2 < GapV)
      {  //we're close to setpoint, use conservative tuning parameters
        myPID2.setOutputLimits(LOW2, HIGH2); // this is the MAX PWM value to move motor, here change in value reflect change in speed of motor.
        myPID2.setCoefficients(consKp, consKi, consKd);
      } else {
      //we're far from setpoint, use aggressive tuning parameters
        myPID2.setOutputLimits(LOW1, HIGH1); // this is the MAX PWM value to move motor, here change in value reflect change in speed of motor.
        myPID2.setCoefficients(aggKp, aggKi, aggKd);
      }
      myPID2.compute();
      PWM_val2 = output2;
      if (PWM_val2 >=0) motorForward2(PWM_val2);
